 ...
 * @LastEditTime: 2021-08-12 11:13:34
 * @LastEditors: Please set LastEditors
 * @Description: In User Settings Edit
 * @FilePath: /template/src/utils/equal.js
 */
import { isString, isObject, toRawType, isNaN } from '@/utils/type';

const isNativeDate = val => val instanceof Date;
const isMapOrSet = val => ['Set', 'Map'].includes(toRawType(val));
...
export function deepEqualWith(customizer, a, b) {
  ...
  if (result === undefined && isObject(val1) && isObject(val2)) {
    if (equalFn(toRawType, [val1, val2]) && equalFn(getSize, [val1, val2])) {
