...
<script>
import EditModal from '../../measureControlEventManagement/modules/EditModal.vue';
...
export default {
  ...
  methods:{
    ...
    handleEdit(record) {
      debugger
      [this.visible, this.type, this.selectedRow] = [true, 'edit', record];
    },
    // 删除
    handleDelete(record) {
      debugger
      this.$confirm({
        title: '提示',
        content: `确认要删除第${record.key}条数据吗？`,
        onOk: () => {
          cksjglDeleteApi(record.key).then(() => {
